...
import {
  ...
  CustomCodeBlockTool,
  CustomFormatTool,
  CustomInlineCodeTool,
  CustomItalicTool,
  CustomLinkTool,
  CustomOrderedListTool,
  CustomUnderlineTool,
  CustomUnorderedListTool
} from './custom-tools';
...
const renderComponentWithCustomTool = (
  <Editor>
    <EditorToolbar
      tools={[
        'bold',
        ...
        {
          renderComponent() {
            return <button type="button">Custom component</button>;
          }
        }
      ]}
    />
...
describe('Toolbar', async () => {
  ...
  it('renders custom tools', async () => {
    const { container, getByText } = render(renderComponentWithCustomTool);
    const toolbar = container.querySelector('.twigs-editor-toolbar')!;
    expect(toolbar).not.toBeNull();
    ...
    const customTool = getByText('Custom component');
    expect(customTool.element()).not.toBeNull();
  });

  it('can override existing tools', async () => {

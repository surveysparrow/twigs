...
Twigs provides a comprehensive color system. Each color has multiple shades from 50 (lightest) to 900 (darkest).
...
```jsx
<Button css={{ backgroundColor: '$primary100' }} >
  Submit
</Button>
```

Below is an example which modifies our primary button using custom css and theme variables. use the `!important` tag if it is necessary.

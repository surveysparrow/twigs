...
### Using `twigs.config.js` (Recommended)

Create `twigs.config.js` file in your project root:
...
Extend your `twigs.config.js` to include dark theme configuration:
...
Import `getCssText` from Twigs and inject it into your document's `<head>`:

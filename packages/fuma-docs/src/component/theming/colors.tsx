...
const ColorGroup: React.FC<ColorGroupProps> = ({ groupName, colors }) => {
  return (
    <div className="mb-8">
      <h3 className="text-lg font-semibold text-fd-foreground mb-4 capitalize">
        {groupName}
      </h3>
...
export function TwigsColorPalette() {
  const organizeColors = () => {
    const colorGroups: Record<
      string,
      Array<{ name: string; value: string; token: string }>
    > = {};
    ...
    Object.keys(colorGroups).forEach((groupName) => {
      if (groupName === "primary" || groupName === "secondary") {
        colorGroups[groupName] = colorGroups[groupName].filter(
